# Опыт переписывания верхнеуровневых абстракций при реализации бота на базе Telegraf.
...
При очередной доработке бота появилась необходимость в сложных цепочках исполнения, например, для последовательных вопросов пользователю и заполнения данных, без захламления кода условиями. Telegraf имеет для этого `Stage` `Scene`, но тайпинги на него отсутствовали на момент реализации. Поэтому было решено переписать `Scene` на том функционале Telegraf, который доступен для лучшего понимания процесса композиции и для общего развития.

## Telegraf и его составные части.
...
`Telegraf` - является расширением класса Composer. Отличается настройками доступа к api.telegram.org и прокси.

`Composer` - класс предоставляющий удобный интерфейс для подписки на события бота следующим образом:
```ts
...
_А что если нам нужно ветвить цепь исполнения? делать доступными/недоступными команды только после определенных действий?_
...
Схематично выполнение будет выглядеть так:
![Схема менеджера сцен](./SceneManager.png)
